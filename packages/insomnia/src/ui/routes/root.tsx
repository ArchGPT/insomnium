    ...
    <NunjucksEnabledProvider>
      ...
      <div className="app">
        ...
        <div className="w-full h-full divide-x divide-solid divide-y divide-[--hl-md] grid-template-app-layout grid relative bg-[--color-bg]">
          <div className='absolute top-2 left-0 text-white w-full text-center'>{!location.pathname.startsWith("/llm") && !workspaceData && <>{_.truncate(window.app.getPath('userData') + "/defaultCodebase/.insomnium", { 'length': Math.floor(window.innerWidth / 8) })}
          ...
          <div className="relative [grid-area:Statusbar] flex items-center justify-between overflow-hidden">
            ...
            <Link>
              ...
              >
                100% local and privacy-focused. With ArchGPT support for local & remote LLMs <Icon className="text-white" icon="heart" />
              </a>
            </Link>
          </div>
        </div>
